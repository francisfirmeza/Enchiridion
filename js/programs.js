...
async function loadPrograms() {
  ...
  if (!programs || programs.length === 0) {
    listEl.innerHTML =
      '<p class="muted">No programs yet. Create one above.</p>';
    return;
  }
...
window.openProgram = async (programId) => {
  ...
  document.getElementById("detail-scheme").textContent =
    prog.progression_scheme;
  ...
  const countByDay = {};
  exercises?.forEach((e) => {
    countByDay[e.day_id] = (countByDay[e.day_id] || 0) + 1;
  });
...
document
  .getElementById("save-exercise-btn")
  .addEventListener("click", async () => {
    ...
    const weight =
      parseFloat(document.getElementById("ex-weight").value) || null;
    ...
    if (!name) {
      errEl.textContent = "Exercise name required.";
      return;
    }
    ...
    ["ex-name", "ex-reps", "ex-weight", "ex-pct", "ex-rpe"].forEach(
...
async function loadExercises(dayId) {
  ...
  if (!exercises || exercises.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="muted">No exercises yet.</td></tr>';
    return;
  }
